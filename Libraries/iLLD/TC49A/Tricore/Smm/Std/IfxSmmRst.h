...
typedef enum
{
    ...
    IfxSmmRst_Trigger_smusec     = 8,  /**< \brief Secure SMU */
...
typedef struct
{
    ...
    IfxSmmRst_TriggerRstCfgType smuSec;           /**< \brief SMU safe1 reset1 trigger configuration */
...
typedef struct
{
    boolean                   forceRstOnTimeout;       /**< \brief TRUE --> Force-start next warm reset internally only when counter (TOUTCNT) expires */
...
typedef struct
{
    ...
    Ifx_SMM_RSTSTATBFW         bootFwReset;                 /**< \brief Reset type status for boot software (RSTSTATBFW) */
...
/** \brief Returns the Boot Firmware reset status
 * \return Boot FW Reset Status register
 */
IFX_INLINE uint32 IfxSmmRst_getBootFwResetStatus(void);
