 ...
 *
 *
 * Structure to store baudrate and clock information
 *
 ...
 */
#define IFXSDMMC_1KLOOPDELAY_US                    ((uint32)((IFXSDMMC_1LOOP_CPUCYCLES * (uint32)1000 * (uint32)1000000) / IfxClock_getCpuFrequency()))
...
/** \brief Time Delay (US) for high speed shift timeout for EMMC cards
 */
#ifndef IFXSDMMC_MMC_DELAY_HIGHSPEED_SHIFT_US
#define IFXSDMMC_MMC_DELAY_HIGHSPEED_SHIFT_US      ((uint32)200)
#endif
...
typedef enum
{
    IfxSdmmc_FrequencyConfigStatus_valid,           /**< \brief Indicates a valid frequency configuration is set in H/w */
    IfxSdmmc_FrequencyConfigStatus_unSupportedFreq  /**< \brief Unsupported frequency configured Note: Configured frequency to be less then Fsdmmc value */
} IfxSdmmc_FrequencyConfigStatus;
...
/** \brief Structure to store baudrate and clock information
 */
typedef struct
{
    uint32 sdmmcClkout;             /**< \brief SDMMC Clock out frequency */
    uint32 sourceFrequency;         /**< \brief Reference Clock Input to SDMMC */
    uint32 maxClockFrequency;       /**< \brief Max Clock for SDMMC */
} IfxSdmmc_Frequencies;
...
typedef struct
{
    ...
    IfxApApu_ApuConfig   apuConfig;          /**< \brief PROTSE Configuration */
} IfxSdmmc_ApConfig;
...
/** \brief Writes data to buffer data register
 * \param sdmmcSFR sdmmcSFR pointer to the base address of SDMCC register space
 * \param configStatus configStatus Pointer to config structure of store baudrate and clock information
 * \return Valid/Invalid Status of the frequencies configured
 */
IFX_EXTERN IfxSdmmc_FrequencyConfigStatus IfxSdmmc_getFrequencyValues(Ifx_SDMMC *sdmmcSFR, IfxSdmmc_Frequencies *configStatus);
 ...
 * \return None
 */
IFX_EXTERN void IfxSdmmc_initApConfig(IfxSdmmc_ApConfig *config);
 ...
 * \return None
 */
IFX_EXTERN void IfxSdmmc_initAp(Ifx_SDMMC *sdmmcSFR, IfxSdmmc_ApConfig *config);

/** \brief Configures access to all masters to all the SDMMC in the device
 * \param apConfig config pointer to configuration structure.
 * \return None
 */
IFX_EXTERN void IfxSdmmc_configureAccessToSdmmc(IfxApApu_ApuConfig *apConfig);
...
IFX_INLINE volatile Ifx_SRC_SRCR *IfxSdmmc_getSrcPointer(Ifx_SDMMC *sdmmcSFR, IfxSdmmc_InterruptType interruptType)
{
    ...
    {
        return &MODULE_SRC.SDMMC[0].ERR;
    }
    else
    {
        return &MODULE_SRC.SDMMC[0].DMA;
    }
}
