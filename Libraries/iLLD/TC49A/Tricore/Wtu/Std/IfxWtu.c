...
uint8 IfxWtu_enableWatchdogWithDebugger(void)
{
    /* TODO Update as per spec */
    boolean status = 0;
    return status;
...
void IfxWtu_initCpuApConfig(IfxWtu_cpuApConfig *config)
{
    IfxApProt_initConfig(&config->cpuProtseConfig);
    IfxApApu_initConfig(&config->cpuApuConfig);
}


void IfxWtu_initCpuAp(Ifx_WTU *wtu, IfxWtu_cpuApConfig *config, uint8 cpuId)
{
    /* Initialize the PROTs */
    IfxApProt_init((Ifx_PROT_PROT *)&wtu->WDTCPU[cpuId].PROTSE, &config->cpuProtseConfig);

    /* Change the state to CONFIG, Configure APU and set PROT state back to RUN */
    /* Initialize the APU */
    IfxApProt_setState((Ifx_PROT_PROT *)&wtu->WDTCPU[cpuId].PROTSE, IfxApProt_State_config);
    IfxApApu_init((Ifx_ACCEN_ACCEN *)&wtu->WDTCPU[cpuId].ACCEN, &config->cpuApuConfig);
    IfxApProt_setState((Ifx_PROT_PROT *)&wtu->WDTCPU[cpuId].PROTSE, IfxApProt_State_run);
}


void IfxWtu_initSystemApConfig(IfxWtu_systemApConfig *config)
{
    IfxApProt_initConfig(&config->systemProtseConfig);
    IfxApApu_initConfig(&config->systemApuConfig);
}


void IfxWtu_initSystemAp(Ifx_WTU *wtu, IfxWtu_systemApConfig *config)
{
    /* Initialize the PROTs */
    IfxApProt_init((Ifx_PROT_PROT *)&wtu->WDTSYS.PROTSE, &config->systemProtseConfig);

    /* Change the state to CONFIG, Configure APU and set PROT state back to RUN */
    /* Initialize the APU */
    IfxApProt_setState((Ifx_PROT_PROT *)&wtu->WDTSYS.PROTSE, IfxApProt_State_config);
    IfxApApu_init((Ifx_ACCEN_ACCEN *)&wtu->WDTSYS.ACCEN, &config->systemApuConfig);
    IfxApProt_setState((Ifx_PROT_PROT *)&wtu->WDTSYS.PROTSE, IfxApProt_State_run);
}


void IfxWtu_initSecureAp(Ifx_WTU *wtu, IfxWtu_secureApConfig *config)
{
    /* Initialize the PROTs */
    IfxApProt_init((Ifx_PROT_PROT *)&wtu->WDTSEC.PROTCSE, &config->secureProtseConfig);

    /* Change the state to CONFIG, Configure APU and set PROT state back to RUN */
    /* Initialize the APU */
    IfxApProt_setState((Ifx_PROT_PROT *)&wtu->WDTSEC.PROTCSE, IfxApProt_State_config);
    IfxApApu_init((Ifx_ACCEN_ACCEN *)&wtu->WDTSEC.ACCEN, &config->secureApuConfig);
    IfxApProt_setState((Ifx_PROT_PROT *)&wtu->WDTSEC.PROTCSE, IfxApProt_State_run);
}


void IfxWtu_initSecureApConfig(IfxWtu_secureApConfig *config)
{
    IfxApProt_initConfig(&config->secureProtseConfig);
    IfxApApu_initConfig(&config->secureApuConfig);
}
