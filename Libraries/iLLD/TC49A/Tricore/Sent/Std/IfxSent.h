...
/** \brief SENT.CH[channelId].IOCR.ETS , Specifies the external (GTM/eGTM) trigger line
 */
typedef enum
{
...
typedef enum
{
    ...
    IfxSent_TriggerSource_externalTrigger = 3  /**< \brief Specifies pulse generation after each external (GTM/eGTM) trigger */
 ...
 */
IFX_INLINE volatile Ifx_SRC_SRCR *IfxSent_getChannelSrc(Ifx_SENT *sent, IfxSent_ChannelId channelId);
...
/** \brief Set External Trigger Select
 * \param sent base address of the SENT register space
 * \param channelId SENT channel number
 * \param triggerLine external (GTM/eGTM) trigger line
 * \return None
 */
IFX_INLINE void IfxSent_setExternalTriggerSelect(Ifx_SENT *sent, IfxSent_ChannelId channelId, IfxSent_ExternalTrigger triggerLine);
 ...
 * \return None
 */
IFX_EXTERN void IfxSent_initApConfig(IfxSent_ApConfig *config);
 ...
 * \return None
 */
IFX_EXTERN void IfxSent_initAp(Ifx_SENT *sent, IfxSent_ApConfig *config);

/** \brief Configures access to all masters to all the SENTs in the device
 * \param apConfig config pointer to configuration structure.
 * \return None
 */
IFX_EXTERN void IfxSent_configureAccessToSents(IfxApApu_ApuConfig *apConfig);
 ...
 */
IFX_EXTERN IfxSent_ChannelStatus IfxSent_getChannelStatus(Ifx_SENT *sent, IfxSent_ChannelId channelId);
...
IFX_INLINE volatile Ifx_SRC_SRCR *IfxSent_getChannelSrc(Ifx_SENT *sent, IfxSent_ChannelId channelId)
{
    ...
    Ifx_SRC_SRCR   *sentSrc = NULL_PTR;

    if (sent == &MODULE_SENT0)
    {
        tempINP.U = MODULE_SENT0.CH[channelId].INP.U;
        sentSrc   = (Ifx_SRC_SRCR *)&(MODULE_SRC.SENT[0].SR[tempINP.B.RSI]);
    }

#if IFXSENT_NUM_MODULES > 1
    else
    {
        tempINP.U = MODULE_SENT1.CH[channelId].INP.U;
        sentSrc   = (Ifx_SRC_SRCR *)&(MODULE_SRC.SENT[1].SR[tempINP.B.RSI]);
    }
#endif
    return sentSrc;
}
...
IFX_INLINE void IfxSent_setExternalTriggerSelect(Ifx_SENT *sent, IfxSent_ChannelId channelId, IfxSent_ExternalTrigger triggerLine)
{
    sent->CH[channelId].IOCR.B.ETS = triggerLine;
    sent->CH[channelId].SCR.B.TRIG = IfxSent_TriggerSource_externalTrigger;
}
