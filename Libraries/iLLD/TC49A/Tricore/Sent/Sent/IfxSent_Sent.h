 ...
 *     sentChannelConfig.interrupt.vmId = IxfSrc_VmId=0;
 *
 *     int i;
 *     for(i=0; i<TESTED_SENT_CHANNELS; ++i)
 *     {
...
typedef struct
{
    ...
    IfxSrc_VmId vmId;              /**< \brief The Virtual Machine which is going to be the interrupt service provider */
...
/** \brief This type defines different operations for the SPC channel transmission
 */
typedef struct
{
    IfxSent_TriggerSource triggerSource;       /**< \brief Trigger source for SPC operation */
    IfxSent_TimeBase      timeBase;            /**< \brief Pulse time base for SPC */
    uint8                 pulseLength;         /**< \brief Length of pulse in tick times */
    uint8                 delay;               /**< \brief SPC pulse delay time */
} IfxSent_Sent_SpcType;
 ...
 * \return None
 */
IFX_EXTERN void IfxSent_Sent_readChannelDataFrame(IfxSent_Sent_Channel *channel, IfxSent_Sent_Frame *frame);
 ...
 * \param channel reads the Serial data recieved and collected over several SENT frames
 * \param config configure the SPC related variable
 * \return TRUE if timeout error. FALSE if pulse generation successful
 */
IFX_EXTERN boolean IfxSent_Sent_spcGenPulse(IfxSent_Sent_Channel *channel, IfxSent_Sent_SpcType *config);
