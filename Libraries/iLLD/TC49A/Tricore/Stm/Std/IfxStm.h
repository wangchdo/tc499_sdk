 ...
 */
IFX_INLINE float32 IfxStm_getFrequency(void);
 ...
 */
IFX_INLINE uint32 IfxStm_getOffsetTimer(Ifx_CPU *cpu, uint8 offset);
 ...
 * \return None
 */
IFX_INLINE void IfxStm_waitTicks(Ifx_CPU *cpu, uint32 ticks);
 ...
 * \return ticks
 */
IFX_INLINE sint32 IfxStm_getTicksFromMicroseconds(uint32 microSeconds);
 ...
 * \return ticks
 */
IFX_INLINE sint32 IfxStm_getTicksFromMilliseconds(uint32 milliSeconds);
 ...
 * \return microseconds
 */
IFX_INLINE uint32 IfxStm_getMicrosecondsFromTicks(uint32 ticks);
 ...
 * \return milliseconds
 */
IFX_INLINE uint32 IfxStm_getMillisecondsFromTicks(uint32 ticks);
...
IFX_INLINE uint32 IfxStm_getCompare(Ifx_CPU *cpu, IfxStm_Comparator comparator)
{
    ...
    Ifx_CPU_STM_CMP *cmp0 = (Ifx_CPU_STM_CMP *)(&cpu->STM.VM[vmId].CMP[0]);
    Ifx_CPU_STM_CMP *cmp1 = (Ifx_CPU_STM_CMP *)(&cpu->STM.VM[vmId].CMP[1]);
#endif
...
IFX_INLINE float32 IfxStm_getFrequency(void)
{
    ...
    result = (float32)IfxClock_getStmFrequency();
...
IFX_INLINE sint32 IfxStm_getTicksFromMicroseconds(uint32 microSeconds)
{
    sint32 freq = (sint32)IfxStm_getFrequency();
...
#else
    uint32           vmId = (uint32)IfxCpu_getVirtualMachine();
    Ifx_CPU_STM_REL *rel  = (Ifx_CPU_STM_REL *)(&cpu->STM.VM[vmId].RELTIM);
#endif
...
{
    sint32 freq = (sint32)IfxStm_getFrequency();
    return (uint32)((1000000 * ticks) / freq);
}
