...
void IfxStm_clearCompareFlag(Ifx_CPU *cpu, IfxStm_Comparator comparator)
{
    ...
    Ifx_CPU_STM_ISCR *iscr = (Ifx_CPU_STM_ISCR *)(&cpu->STM.VM[vmId].ISCR);
#endif
...
#else
    uint32           vmId = (uint32)IfxCpu_getVirtualMachine();
    Ifx_CPU_STM_ICR *icr  = (Ifx_CPU_STM_ICR *)(&cpu->STM.VM[vmId].STM_ICR);
#endif
...
boolean IfxStm_initCompare(Ifx_CPU *cpu, const IfxStm_CompareConfig *config)
{
...
#else
    uint32             vmId   = (uint32)IfxCpu_getVirtualMachine();
    Ifx_CPU_STM_CMCON *comcon = (Ifx_CPU_STM_CMCON *)(&cpu->STM.VM[vmId].CMCON);
    Ifx_CPU_STM_ICR   *icr    = (Ifx_CPU_STM_ICR *)(&cpu->STM.VM[vmId].STM_ICR);
    ...
    if (config->comparator == IfxStm_Comparator_0)
    {
        cmp0->U = IfxStm_getOffsetTimer(cpu, (uint8)config->compareOffset);
    }
    else if (config->comparator == IfxStm_Comparator_1)
    {
        cmp1->U = IfxStm_getOffsetTimer(cpu, (uint8)config->compareOffset);
    }
    else
    {
        /*Invalid value */
    ...
    if (config->comparator == IfxStm_Comparator_0)
    {
        cmp0->U = IfxStm_getOffsetTimer(cpu, (uint8)config->compareOffset) + config->ticks;
    }
    else if (config->comparator == IfxStm_Comparator_1)
    {
        cmp1->U = IfxStm_getOffsetTimer(cpu, (uint8)config->compareOffset) + config->ticks;
    }
    else
    {
        /*Invalid value */
...
#else
    uint32           vmId = (uint32)IfxCpu_getVirtualMachine();
    Ifx_CPU_STM_ISR *isr  = (Ifx_CPU_STM_ISR *)(&cpu->STM.VM[vmId].ISR);
#endif
