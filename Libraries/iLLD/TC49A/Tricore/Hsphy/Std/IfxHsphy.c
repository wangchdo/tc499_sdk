...
IFX_CONST uint32 IfxHsphy_Sgmii_paramConfig[IfxHsphy_SgmiiSpeed_max][IfxHsphy_XpcsRefClk_max][IfxHsphy_SgmiiParamIndex_maxParamIndex] = {
    {
        {50,  3, 0, 5, 10, 0,   1000, 10, 1, 0, 0, 2, 2, 17, 1, 1, 15, 0}, /* Parameter values for 25MHz SGMII 0P1G mode */
        {125, 2, 0, 6, 12, 0,   1000, 16, 1, 0, 0, 2, 2, 17, 1, 1, 15, 0}, /* Parameter values for 40MHz SGMII 0P1G mode */
        {50,  3, 0, 6, 12, 0,   1000, 20, 2, 0, 0, 2, 2, 17, 1, 1, 15, 0}  /* Parameter values for 100MHz SGMII 0P1G mode */
    },                                                                     /* values for SGMII 0P1G mode */
    {
        {50,  1, 0, 5, 10, 0,   1000, 10, 1, 0, 0, 2, 2, 1,  1, 1, 15, 0}, /* Parameter values for 25MHz SGMII 1G mode */
        {125, 0, 0, 6, 12, 0,   1000, 16, 1, 0, 0, 2, 2, 0,  1, 1, 15, 0}, /* Parameter values for 40MHz SGMII 1G mode */
        {100, 0, 0, 6, 12, 0,   1000, 20, 2, 0, 0, 2, 2, 0,  1, 1, 15, 0}  /* Parameter values for 100MHz SGMII 1G mode */
    },                                                                     /* values for SGMII 1G mode. */
    {
        {125, 0, 0, 5, 10, 0,   1000, 8,  1, 0, 0, 1, 1, 0,  1, 1, 15, 0}, /* Parameter values for 25MHz SGMII 2P5G mode */
        {39,  1, 0, 4, 8,  260, 1016, 13, 1, 0, 0, 1, 1, 0,  1, 1, 15, 1}, /* Parameter values for 40MHz SGMII 2P5G mode */
        {125, 0, 0, 7, 14, 0,   1000, 16, 2, 0, 0, 1, 1, 0,  1, 1, 15, 1}  /* Parameter values for 100MHz SGMII 2P5G mode */
    },                                                                     /* values for SGMII 2P5G mode. */

    {
        {125, 0, 0, 5, 10, 0,   1000, 8,  1, 0, 0, 0, 0, 2,  1, 1, 15, 0}, /*  Parameter values for 25MHz SGMII 5G mode */
        {39,  1, 0, 4, 8,  260, 1016, 13, 1, 0, 0, 0, 0, 0,  1, 1, 15, 1}, /*  Parameter values for 40MHz SGMII 5G mode */
        {125, 0, 0, 7, 14, 0,   1000, 16, 2, 0, 0, 0, 0, 0,  1, 1, 15, 1}  /*  Parameter values for 100MHz SGMII 5G mode */
    }
};
...
IFX_CONST uint32 IfxHsphy_Usxgmii_paramConfig[][IfxHsphy_XpcsRefClk_max][IfxHsphy_UsxgmiiParamIndex_maxParamIndex] = {
    {
        {103, 3, 1, 5,  10, 390, 1031, 10, 1, 1, 1, 1, 0, 3, 3, 3, 12, 1}, /* Parameter values for 25MHz USXGMII 5G mode */
        {1,   3, 1, 11, 10, 636, 1031, 16, 1, 1, 1, 1, 0, 3, 3, 3, 12, 1}, /* Parameter values for 40MHz USXGMII 5G mode */
        {103, 3, 1, 10, 10, 390, 1031, 20, 2, 1, 1, 1, 0, 3, 3, 3, 12, 1} /* Parameter values for 100MHz USXGMII 5G mode */
    }
};
...
boolean IfxHsphy_resetModule(Ifx_HSPHY *hsphyRegPtr)
{
    ...
    while (hsphyRegPtr->RST.STAT.B.KRST == 1)   /* Wait until KRST is cleared, only after this reset sequence is completed */
    {}
...
boolean IfxHsphy_setPhyRefClockSel(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_PhyIndex phyIndex, IfxHsphy_TrgtDevice device)
{
    ...
    if (phyIndex == IfxHsphy_PhyIndex_0)
    {
        ...
        {
            //PCIE
            hsphyRegPtr->CTRL.PHY.B.PRS |= 1U; /* 0bXX1, IFXHSPHY_PHY0_REFCLKSEL_PAD */
        }
        else if (device == IfxHsphy_TrgtDevice_xgmac)
        {
            //XGMAC
            hsphyRegPtr->CTRL.PHY.B.PRS &= 6U; /* 0bXX0, IFXHSPHY_PHY0_REFCLKSEL_ALT */
        }
        else
        {
            ret = FALSE;
    ...
    if (phyIndex == IfxHsphy_PhyIndex_1)
    {
        ...
        {
            //XGMAC
            hsphyRegPtr->CTRL.PHY.B.PRS &= 5U; /* 0bX0X, IFXHSPHY_PHY1_REFCLKSEL_ALT */
        }
        else if (device == IfxHsphy_TrgtDevice_trace)
        {
            //TPCS
            hsphyRegPtr->CTRL.PHY.B.PRS |= 2U; /* 0bX1X, IFXHSPHY_PHY1_REFCLKSEL_PAD */
        }
        else
        {
            ret = FALSE;
...
boolean IfxHsphy_selFunction(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_PhyIndex phyIndex, IfxHsphy_TrgtDevice device)
{
    ...
    if (phyIndex == IfxHsphy_PhyIndex_0)
    {
        ...
        {
            //PCIE
            hsphyRegPtr->CTRL.PHY.B.FSP0 = 1;
        }
        else if (device == IfxHsphy_TrgtDevice_xgmac)
        {
            //XGMAC
            hsphyRegPtr->CTRL.PHY.B.FSP0 = 0;
        }
        else
        {
            ret = FALSE;
    ...
    if (phyIndex == IfxHsphy_PhyIndex_1)
    {
        ...
        {
            //XGMAC
            hsphyRegPtr->CTRL.PHY.B.FSP1 = 0;
        }
        else if (device == IfxHsphy_TrgtDevice_trace)
        {
            //TPCS
            hsphyRegPtr->CTRL.PHY.B.FSP1 = 1;
        }
        else
        {
            ret = FALSE;
...
boolean IfxHsphy_isPhyReady(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_PhyIndex phyIndex)
{
    ...
    if (phyIndex == IfxHsphy_PhyIndex_0)
    {
        ...
        ret = (hsphyRegPtr->CTRL.PHYMEM.B.INITDONE0 == 1);
    }
    else if (phyIndex == IfxHsphy_PhyIndex_1)
    {
        ret = (hsphyRegPtr->CTRL.PHYMEM.B.INITDONE1 == 1);
    }
    else
    {
        ret = FALSE;
...
boolean IfxHsphy_setExtLdDone(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_PhyIndex phyIndex)
{
    ...
    {
        //Check if the PHY is ready by checking if SRAM loading start.
        if (hsphyRegPtr->CTRL.PHYMEM.B.EXTLDDONE0 != 1)
        {
            hsphyRegPtr->CTRL.PHYMEM.B.EXTLDDONE0 = 1;
        }
    ...
    {
        //Check if the PHY is ready by checking if SRAM loading start.
        if (hsphyRegPtr->CTRL.PHYMEM.B.EXTLDDONE1 != 1)
        {
            hsphyRegPtr->CTRL.PHYMEM.B.EXTLDDONE1 = 1;
        }
...
boolean IfxHsphy_resetXpcs(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_GethXpcsParams *xpcs)
{
    uint32 timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    uint8  timeOutError      = 0U;
...
IfxHsphy_Geth_SgmiiSpeedConfigStatus IfxHsphy_Geth_setSgmiiSpeedMode(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_TrgtDeviceSpeed trgtSpeed, IfxHsphy_GethXpcsParams *xpcs)
{
    uint32              timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    uint8               timeOutError      = 0U;
...
boolean IfxHsphy_Geth_setUsxgmiiSpeedMode(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_TrgtDeviceSpeed trgtSpeed, IfxHsphy_GethXpcsParams *xpcs)
{
    uint32                timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    uint8                 timeOutError      = 0U;
...
boolean IfxHsphy_Geth_rgmiiInit(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_Geth_RgmiiCfg *config)
{
    ...
    uint32 timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    uint8  timeOutError      = 0U;
    hsphyRegPtr->CTRL.ETH.B.FSR   = 1;                            //To select 1:RGMII pads for XGMAC1,0:RGMII pads for XSPI
...
void IfxHsphy_Geth_setRgmiiPins(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_Geth_RgmiiCfg *config)
{
    IfxHsphy_Geth_setupRgmiiInputPins(hsphyRegPtr, config->rgmiiPins);
    IfxHsphy_Geth_setupRgmiiOutputPins(hsphyRegPtr, config->rgmiiPins);
}


void IfxHsphy_Geth_setMiiPins(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_Geth_MiiCfg *config)
{
    IfxHsphy_Geth_setupMiiInputPins(hsphyRegPtr, config->miiPins);
    IfxHsphy_Geth_setupMiiOutputPins(hsphyRegPtr, config->miiPins);
}


void IfxHsphy_Geth_setRmiiPins(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_Geth_RmiiCfg *config)
{
    IfxHsphy_Geth_setupRmiiInputPins(hsphyRegPtr, config->rmiiPins);
...
void IfxHsphy_Geth_setupRmiiOutputPins(Ifx_HSPHY *hsphyRegPtr, const IfxHsphy_Geth_RmiiPins *rmiiPins)
{
    ...
    if (rmiiPins != NULL_PTR)
    {
        if (rmiiPins->mdc != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(mdc->pin.port, mdc->pin.pinIndex);
        ...
        if (rmiiPins->txEn != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txen->pin.port, txen->pin.pinIndex);
        ...
        if (rmiiPins->txd0 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txd0->pin.port, txd0->pin.pinIndex);
        ...
        if (rmiiPins->txd1 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txd1->pin.port, txd1->pin.pinIndex);
...
void IfxHsphy_Geth_setupRmiiInputPins(Ifx_HSPHY *hsphyRegPtr, const IfxHsphy_Geth_RmiiPins *rmiiPins)
{
    ...
    {
        hsphyRegPtr->CTRL.ETH.B.RXCTL = crsDiv->select;
        IfxHsphy_setPinControllerSelection(crsDiv->pin.port, crsDiv->pin.pinIndex);
    ...
    {
        hsphyRegPtr->CTRL.ETH.B.RXCLK = refClk->select;
        IfxHsphy_setPinControllerSelection(refClk->pin.port, refClk->pin.pinIndex);
    ...
    if (rmiiPins->rxd0 != NULL_PTR)
    {
        ...
        IfxHsphy_setPinControllerSelection(rxd0->pin.port, rxd0->pin.pinIndex);
    ...
    if (rmiiPins->rxd1 != NULL_PTR)
    {
        ...
        IfxHsphy_setPinControllerSelection(rxd1->pin.port, rxd1->pin.pinIndex);
    ...
    if (rmiiPins->mdio0 != NULL_PTR)
    {
        IfxGeth_Mdio_InOut *mdio0 = rmiiPins->mdio0;
        hsphyRegPtr->CTRL.ETH.B.MDIO0 = mdio0->inSelect;
        IfxHsphy_setPinControllerSelection(mdio0->pin.port, mdio0->pin.pinIndex);
        IfxPort_setPinModeInput(mdio0->pin.port, mdio0->pin.pinIndex, mode);
        IfxPort_setPinPadDriver(mdio0->pin.port, mdio0->pin.pinIndex, speedGrade);
    }

    if (rmiiPins->mdio1 != NULL_PTR)
    {
        IfxGeth_Mdio_InOut *mdio1 = rmiiPins->mdio1;
        hsphyRegPtr->CTRL.ETH.B.MDIO1 = mdio1->inSelect;
        IfxHsphy_setPinControllerSelection(mdio1->pin.port, mdio1->pin.pinIndex);
        IfxPort_setPinModeInput(mdio1->pin.port, mdio1->pin.pinIndex, mode);
        IfxPort_setPinPadDriver(mdio1->pin.port, mdio1->pin.pinIndex, speedGrade);
    }
}
...
void IfxHsphy_Xspi_setXspiPins(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_Xspi_XspiCfg *config)
{
    ...
    if (pins != NULL_PTR)
    {
        if (pins->clk != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(clk->pin.port, clk->pin.pinIndex);
        ...
        if (pins->clk_n != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(clk_n->pin.port, clk_n->pin.pinIndex);
        ...
        if (pins->cs != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(cs->pin.port, cs->pin.pinIndex);
        ...
        if (pins->txd2 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txd2->pin.port, txd2->pin.pinIndex);
        ...
        if (pins->txd3 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txd3->pin.port, txd3->pin.pinIndex);
        ...
        if (pins->txd4 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txd4->pin.port, txd4->pin.pinIndex);
        ...
        if (pins->txd5 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txd5->pin.port, txd5->pin.pinIndex);
        ...
        if (pins->txd6 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txd6->pin.port, txd6->pin.pinIndex);
        ...
        if (pins->txd7 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txd7->pin.port, txd7->pin.pinIndex);
        ...
        if (pins->txdm != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txdm->pin.port, txdm->pin.pinIndex);
...
void IfxHsphy_Geth_setupRgmiiInputPins(Ifx_HSPHY *hsphyRegPtr, const IfxHsphy_Geth_RgmiiPins *rgmiiPins)
{
    IfxPort_InputMode mode       = IfxPort_InputMode_noPullDevice;
    IfxPort_PadDriver speedGrade = IfxPort_PadDriver_cmosAutomotiveSpeed3;
    ...
    if (rgmiiPins != NULL_PTR)
    {
        if (rgmiiPins->rxClk != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(rxClk->pin.port, rxClk->pin.pinIndex);
        ...
        if (rgmiiPins->rxCtl != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(rxCtl->pin.port, rxCtl->pin.pinIndex);
        ...
        if (rgmiiPins->rxd2 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(rxd2->pin.port, rxd2->pin.pinIndex);
        ...
        if (rgmiiPins->rxd3 != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(rxd3->pin.port, rxd3->pin.pinIndex);
        ...
        if (rgmiiPins->grefClk != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(grefClk->pin.port, grefClk->pin.pinIndex);
        ...
        if (rgmiiPins->mdio0 != NULL_PTR)
        {
            IfxGeth_Mdio_InOut *mdio0 = rgmiiPins->mdio0;
            hsphyRegPtr->CTRL.ETH.B.MDIO0 = mdio0->inSelect;
            IfxHsphy_setPinControllerSelection(mdio0->pin.port, mdio0->pin.pinIndex);
            IfxPort_setPinModeInput(mdio0->pin.port, mdio0->pin.pinIndex, mode);
            IfxPort_setPinPadDriver(mdio0->pin.port, mdio0->pin.pinIndex, speedGrade);
        ...
        if (rgmiiPins->mdio1 != NULL_PTR)
        {
            IfxGeth_Mdio_InOut *mdio1 = rgmiiPins->mdio1;
            hsphyRegPtr->CTRL.ETH.B.MDIO1 = mdio1->inSelect;
            IfxHsphy_setPinControllerSelection(mdio1->pin.port, mdio1->pin.pinIndex);
            IfxPort_setPinModeInput(mdio1->pin.port, mdio1->pin.pinIndex, mode);
            IfxPort_setPinPadDriver(mdio1->pin.port, mdio1->pin.pinIndex, speedGrade);
...
void IfxHsphy_Geth_setupRgmiiOutputPins(Ifx_HSPHY *hsphyRegPtr, const IfxHsphy_Geth_RgmiiPins *rgmiiPins)
{
    ...
    if (rgmiiPins != NULL_PTR)
    {
        ...
        if (rgmiiPins->txClk != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txClk->pin.port, txClk->pin.pinIndex);
        ...
        if (rgmiiPins->txCtl != NULL_PTR)
        {
            ...
            IfxHsphy_setPinControllerSelection(txCtl->pin.port, txCtl->pin.pinIndex);
...
void IfxHsphy_Geth_setupMiiInputPins(Ifx_HSPHY *hsphyRegPtr, const IfxHsphy_Geth_MiiPins *miiPins)
{
    IfxPort_InputMode mode       = IfxPort_InputMode_noPullDevice;
    IfxPort_PadDriver speedGrade = IfxPort_PadDriver_cmosAutomotiveSpeed1;
    ...
    if (miiPins != NULL_PTR)
    {
        if (miiPins->crs != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.CRS = crs->select;
            IfxHsphy_resetPinControllerSelection(crs->pin.port, crs->pin.pinIndex);
        ...
        if (miiPins->col != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.COL = col->select;
            IfxHsphy_resetPinControllerSelection(col->pin.port, col->pin.pinIndex);
        ...
        if (miiPins->txClk != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.TXCLK = txClk->select;
            IfxHsphy_resetPinControllerSelection(txClk->pin.port, txClk->pin.pinIndex);
        ...
        if (miiPins->rxClk != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.RXCLK = rxClk->select;
            IfxHsphy_resetPinControllerSelection(rxClk->pin.port, rxClk->pin.pinIndex);
            IfxPort_setPinModeInput(rxClk->pin.port, rxClk->pin.pinIndex, mode);
            IfxPort_setPinPadDriver(rxClk->pin.port, rxClk->pin.pinIndex, speedGrade);
        }
        ...
        if (miiPins->rxDv != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.RXCTL = rxDv->select;
            IfxHsphy_resetPinControllerSelection(rxDv->pin.port, rxDv->pin.pinIndex);
        ...
        if (miiPins->rxEr != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.RXER = rxEr->select;
            IfxHsphy_resetPinControllerSelection(rxEr->pin.port, rxEr->pin.pinIndex);
        ...
        if (miiPins->rxd0 != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.RXD0 = rxd0->select;
            IfxHsphy_resetPinControllerSelection(rxd0->pin.port, rxd0->pin.pinIndex);
            IfxPort_setPinModeInput(rxd0->pin.port, rxd0->pin.pinIndex, mode);
            IfxPort_setPinPadDriver(rxd0->pin.port, rxd0->pin.pinIndex, speedGrade);
        }
        ...
        if (miiPins->rxd1 != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.RXD1 = rxd1->select;
            IfxHsphy_resetPinControllerSelection(rxd1->pin.port, rxd1->pin.pinIndex);
            IfxPort_setPinModeInput(rxd1->pin.port, rxd1->pin.pinIndex, mode);
            IfxPort_setPinPadDriver(rxd1->pin.port, rxd1->pin.pinIndex, speedGrade);
        }
        ...
        if (miiPins->rxd2 != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.RXD2 = rxd2->select;
            IfxHsphy_resetPinControllerSelection(rxd2->pin.port, rxd2->pin.pinIndex);
            IfxPort_setPinModeInput(rxd2->pin.port, rxd2->pin.pinIndex, mode);
            IfxPort_setPinPadDriver(rxd2->pin.port, rxd2->pin.pinIndex, speedGrade);
        }
        ...
        if (miiPins->rxd3 != NULL_PTR)
        {
            ...
            hsphyRegPtr->CTRL.ETH.B.RXD3 = rxd3->select;
            IfxHsphy_resetPinControllerSelection(rxd3->pin.port, rxd3->pin.pinIndex);
            IfxPort_setPinModeInput(rxd3->pin.port, rxd3->pin.pinIndex, mode);
            IfxPort_setPinPadDriver(rxd3->pin.port, rxd3->pin.pinIndex, speedGrade);
        }

        if (miiPins->mdio0 != NULL_PTR)
        {
            IfxGeth_Mdio_InOut *mdio0 = miiPins->mdio0;
            hsphyRegPtr->CTRL.ETH.B.MDIO0 = mdio0->inSelect;
            IfxHsphy_setPinControllerSelection(mdio0->pin.port, mdio0->pin.pinIndex);
            IfxPort_setPinModeInput(mdio0->pin.port, mdio0->pin.pinIndex, mode);
            IfxPort_setPinPadDriver(mdio0->pin.port, mdio0->pin.pinIndex, speedGrade);
        ...
        if (miiPins->mdio1 != NULL_PTR)
        {
            IfxGeth_Mdio_InOut *mdio1 = miiPins->mdio1;
            hsphyRegPtr->CTRL.ETH.B.MDIO1 = mdio1->inSelect;
            IfxHsphy_setPinControllerSelection(mdio1->pin.port, mdio1->pin.pinIndex);
            IfxPort_setPinModeInput(mdio1->pin.port, mdio1->pin.pinIndex, mode);
            IfxPort_setPinPadDriver(mdio1->pin.port, mdio1->pin.pinIndex, speedGrade);
...
void IfxHsphy_Geth_setupMiiOutputPins(Ifx_HSPHY *hsphyRegPtr, const IfxHsphy_Geth_MiiPins *miiPins)
{
    ...
    // The IfxHsphy_setPinControllerSelection() API call is disabled  for TXD0, TXD1 and TXEN because if enabled data cant be passed to port.
    //The IfxHsphy_resetPinControllerSelection() solves this issues Proper programming seq to be confirmed
    ...
    if (miiPins != NULL_PTR)
    {
        if (miiPins->txEn != NULL_PTR)
        {
            ...
            //IfxHsphy_setPinControllerSelection(txEn->pin.port, txEn->pin.pinIndex);
            IfxHsphy_resetPinControllerSelection(txEn->pin.port, txEn->pin.pinIndex);
            IfxPort_setPinModeOutput(txEn->pin.port, txEn->pin.pinIndex, mode, txEn->select);
            IfxPort_setPinPadDriver(txEn->pin.port, txEn->pin.pinIndex, speedGrade);
        }
        ...
        if (miiPins->txEr != NULL_PTR)
        {
            ...
            IfxHsphy_resetPinControllerSelection(txEr->pin.port, txEr->pin.pinIndex);
        ...
        if (miiPins->txd0 != NULL_PTR)
        {
            ...
            //IfxHsphy_setPinControllerSelection(txd0->pin.port, txd0->pin.pinIndex);
            IfxHsphy_resetPinControllerSelection(txd0->pin.port, txd0->pin.pinIndex);
            IfxPort_setPinModeOutput(txd0->pin.port, txd0->pin.pinIndex, mode, txd0->select);
            IfxPort_setPinPadDriver(txd0->pin.port, txd0->pin.pinIndex, speedGrade);
        }
        ...
        if (miiPins->txd1 != NULL_PTR)
        {
            ...
            //IfxHsphy_setPinControllerSelection(txd1->pin.port, txd1->pin.pinIndex);
            IfxHsphy_resetPinControllerSelection(txd1->pin.port, txd1->pin.pinIndex);
            IfxPort_setPinModeOutput(txd1->pin.port, txd1->pin.pinIndex, mode, txd1->select);
            IfxPort_setPinPadDriver(txd1->pin.port, txd1->pin.pinIndex, speedGrade);
        }
        ...
        if (miiPins->txd2 != NULL_PTR)
        {
            ...
            IfxHsphy_resetPinControllerSelection(txd2->pin.port, txd2->pin.pinIndex);
            IfxPort_setPinModeOutput(txd2->pin.port, txd2->pin.pinIndex, mode, txd2->select);
            IfxPort_setPinPadDriver(txd2->pin.port, txd2->pin.pinIndex, speedGrade);
        }
        ...
        if (miiPins->txd3 != NULL_PTR)
        {
            ...
            IfxHsphy_resetPinControllerSelection(txd3->pin.port, txd3->pin.pinIndex);
            IfxPort_setPinModeOutput(txd3->pin.port, txd3->pin.pinIndex, mode, txd3->select);
            IfxPort_setPinPadDriver(txd3->pin.port, txd3->pin.pinIndex, speedGrade);
        }
...
{
    Ifx_HSPHY_CTRL_PHYREG_CTRL phyControlRegister;
    phyControlRegister.U                    = 0;
    phyControlRegister.B.ADDR               = (Ifx_UReg_32Bit)(regAddr << 1);
    phyControlRegister.B.TYPE               = 0x01;
    phyControlRegister.B.ACT                = 0x01;
    hsphyRegPtr->CTRL.PHYREG[phyIdx].CTRL.U = phyControlRegister.U;

    uint32  timeoutCycleCount = IFXHSPHY_MAX_TIMEOUT;
    boolean timeOutError      = FALSE;

    while (hsphyRegPtr->CTRL.PHYREG[phyIdx].CTRL.B.ACT == 1)
    {
        IFXHSPHY_LOOP_TIMEOUT_CHECK(timeoutCycleCount, timeOutError);
    }
    ...
    if (timeOutError == 0)
    {
        Ifx_HSPHY_CTRL_PHYREG_DATA phyDataRegister;
        phyDataRegister.U = 0;
        phyDataRegister.U = hsphyRegPtr->CTRL.PHYREG[phyIdx].DATA.U;
...
boolean IfxHsphy_Phy_writeMDIORegister(Ifx_HSPHY *hsphyRegPtr, IfxHsphy_PhyIndex phyIdx, uint32 regAddr, uint32 data)
{
    ...
    hsphyRegPtr->CTRL.PHYREG[phyIdx].DATA.U = phyDataRegister.U;

    Ifx_HSPHY_CTRL_PHYREG_CTRL phyControlRegister;
    phyControlRegister.U                    = 0;
    phyControlRegister.B.ADDR               = (Ifx_UReg_32Bit)(regAddr << 1);
    phyControlRegister.B.TYPE               = 0x02;
    phyControlRegister.B.ACT                = 0x01;
    hsphyRegPtr->CTRL.PHYREG[phyIdx].CTRL.U = phyControlRegister.U;
...
void IfxHsphy_Geth_setupPpsOutputPin(Ifx_HSPHY *hsphyRegPtr, IfxGeth_Pps_Out *ppsPin)
{
    ...
    IfxPort_OutputMode mode       = IfxPort_OutputMode_pushPull;
    IfxPort_PadDriver  speedGrade = IfxPort_PadDriver_cmosAutomotiveSpeed3;
    IfxHsphy_setPinControllerSelection(ppsPin->pin.port, ppsPin->pin.pinIndex);
...
void IfxHsphy_Geth_setMdioOnlyPins(Ifx_HSPHY *hsphyRegPtr, const IfxHsphy_Geth_MdioPins *mdioPins)
{
    ...
    {
        IfxHsphy_setPinControllerSelection(mdio->pin.port, mdio->pin.pinIndex);
        ...
        if (mdioPins->portIndex == IfxHsphy_Mdio_PortIndex_0)
        {
            hsphyRegPtr->CTRL.ETH.B.MDIO0 = mdio->inSelect;
        }
        else if (mdioPins->portIndex == IfxHsphy_Mdio_PortIndex_1)
        {
            hsphyRegPtr->CTRL.ETH.B.MDIO1 = mdio->inSelect;
        }
    }
}


void IfxHsphy_setPinControllerSelection(Ifx_P *port, uint8 pinIndex)
{
    if ((port == &MODULE_P16) && ((pinIndex == 12) || (pinIndex == 13)))
    {
        IfxPort_setPinControllerSelection(port, (pinIndex - 1));
    }
    else
    {
        IfxPort_setPinControllerSelection(port, pinIndex);
    }
}


void IfxHsphy_resetPinControllerSelection(Ifx_P *port, uint8 pinIndex)
{
    if ((port == &MODULE_P16) && ((pinIndex == 12) || (pinIndex == 13)))
    {
        IfxPort_resetPinControllerSelection(port, (pinIndex - 1));
    }
    else
    {
        IfxPort_resetPinControllerSelection(port, pinIndex);
    }
}
