...
void IfxPcieCap_configureRasDpErrInjection(Ifx_PCIE_DSP_VSECRASCAP *rasdpSFR, uint8 errorLocation, uint8 count, IfxPcieCap_RasDpErrorType type)
{
    Ifx_PCIE_DSP_VSECRASCAP_RASDP_ERROR_INJ_CTRL_OFF errorInjCtrl;
    errorInjCtrl.U                       = rasdpSFR->RASDP_ERROR_INJ_CTRL_OFF.U;

    errorInjCtrl.B.ERROR_INJ_LOC         = (uint32)errorLocation;
    errorInjCtrl.B.ERROR_INJ_COUNT       = (uint32)count;
    errorInjCtrl.B.ERROR_INJ_TYPE        = (uint32)type;

    rasdpSFR->RASDP_ERROR_INJ_CTRL_OFF.U = errorInjCtrl.U;
}


void IfxPcieCap_enableRasDpErrorInjection(Ifx_PCIE_DSP_VSECRASCAP *rasdpSFR)
{
    rasdpSFR->RASDP_ERROR_INJ_CTRL_OFF.B.ERROR_INJ_EN = 1u;
}
