...
/** \brief Enable RASDP Error Injection to start
 * \param rasdpSFR Ptr to RASDP SFR capability structure
 * \param errorLocation Selects where error injection takes place
 * \param count Error injection count
 * \param type Error injection type
 * \return None
 */
IFX_EXTERN void IfxPcieCap_configureRasDpErrInjection(Ifx_PCIE_DSP_VSECRASCAP *rasdpSFR, uint8 errorLocation, uint8 count, IfxPcieCap_RasDpErrorType type);

/** \brief Enable RASDP Error Injection to start
 * \param rasdpSFR Ptr to RASDP SFR capability structure
 * \return None
 */
IFX_EXTERN void IfxPcieCap_enableRasDpErrorInjection(Ifx_PCIE_DSP_VSECRASCAP *rasdpSFR);
