        ...
        && (pmeEvent < IfxPcie_PmeEvent_count))
    {
        ...
        else
        {
            ...
            case IfxPcie_PmeEvent_turnOffAck:
                eventOccurred = (pcieSFR->DSP.PM.CTRL.B.TOACK == 1u) ? TRUE : FALSE;
                break;
            case IfxPcie_PmeEvent_turnOff:
                eventOccurred = (pcieSFR->DSP.PM.CTRL.B.TO == 1u) ? TRUE : FALSE;
                break;
            default:
                break;
            }
...
boolean IfxPcie_getPendingPmeInterrupts(IfxPcie *pcieSFR, IfxPcie_PmeInterrupt *pmeInterrupt)
{
    ...
    if (((pmStatus1Reg.U) | (pmCtrl.U)) != 0x0u)
    {
        ...
        pmeInterrupt->B.turnOffAcknowledge = (pmCtrl.B.TOACK == 1u) ? TRUE : FALSE;
        pmeInterrupt->B.turnOff            = (pmCtrl.B.TO == 1u) ? TRUE : FALSE;
    }
...
{
    pcieSFR->DSP.PM.INTMSK.U |= enableMask;
}


void IfxPcie_disablePmeInterrupts(IfxPcie *pcieSFR, uint32 disableMask)
{
    pcieSFR->DSP.PM.INTMSK.U &= ~disableMask;
}
...
boolean IfxPcie_getErrorEventStatus(IfxPcie *pcieSFR, IfxPcie_ErrorEvent errorEvent)
{
        ...
        && (errorEvent < IfxPcie_ErrorEvent_count))
    {
        switch (errorEvent)
        {
        case IfxPcie_ErrorEvent_rcError:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.RCERRINT == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_safCorr:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.SAFCORR == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_safUncorr:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.SAFUNCORR == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_sysError:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.SYSERR == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_corr:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.CORR == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_nonFatal:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.NFAT == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_fatal:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.FAT == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_a2sData:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.A2SDAT == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_a2sAddr:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.A2SADDR == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_s2aData:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.S2ADAT == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_s2aAddr:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.S2ADAT == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_msterrm:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.MSTERRM == 1u) ? TRUE : FALSE;
            break;
        case IfxPcie_ErrorEvent_slverrm:
            eventOccurred = (pcieSFR->DSP.ERR.CTRL.B.SLVERRM == 1u) ? TRUE : FALSE;
            break;
        default:
            break;
        }
...
{
    boolean               eventOccurred = FALSE;
    Ifx_PCIE_DSP_ERR_CTRL errorCtrl;
    errorCtrl.U = pcieSFR->DSP.ERR.CTRL.U;

    if (((errorCtrl.U) & ~(0x00003000u)) != 0x0u)       /* Ignore the RASDP events */
    {
        eventOccurred               = TRUE;
        errorInterrupt->B.rcError   = (errorCtrl.B.RCERRINT == 1u) ? TRUE : FALSE;
        errorInterrupt->B.safCorr   = (errorCtrl.B.SAFCORR == 1u) ? TRUE : FALSE;
        errorInterrupt->B.safUncorr = (errorCtrl.B.SAFUNCORR == 1u) ? TRUE : FALSE;
        errorInterrupt->B.sysError  = (errorCtrl.B.SYSERR == 1u) ? TRUE : FALSE;
        errorInterrupt->B.corr      = (errorCtrl.B.CORR == 1u) ? TRUE : FALSE;
        errorInterrupt->B.nonFatal  = (errorCtrl.B.NFAT == 1u) ? TRUE : FALSE;
        errorInterrupt->B.fatal     = (errorCtrl.B.FAT == 1u) ? TRUE : FALSE;
        errorInterrupt->B.a2sData   = (errorCtrl.B.A2SDAT == 1u) ? TRUE : FALSE;
        errorInterrupt->B.a2sAddr   = (errorCtrl.B.A2SADDR == 1u) ? TRUE : FALSE;
        errorInterrupt->B.s2aData   = (errorCtrl.B.S2ADAT == 1u) ? TRUE : FALSE;
        errorInterrupt->B.s2aAddr   = (errorCtrl.B.S2ADAT == 1u) ? TRUE : FALSE;
    }
...
{
    pcieSFR->DSP.ERR.INTMSK.U |= enableMask;
}


void IfxPcie_disableErrorInterrupts(IfxPcie *pcieSFR, uint32 disableMask)
{
    pcieSFR->DSP.ERR.INTMSK.U &= ~disableMask;
}
...
void IfxPcie_waitForHsphy(Ifx_HSPHY *hsphySFR, IfxHsphy_PhyIndex phyIndex)
{
    ...
    while (calibDone == 0)
    {
        hsphySFR->CTRL.PHYREG[(uint32)phyIndex].CTRL.U = 0x000A4050u;   //Read Access to RAWCMN_DIG_AON_CMNCAL_STATUS

        while (1 == hsphySFR->CTRL.PHYREG[(uint32)phyIndex].CTRL.B.ACT) //wait until finish reading
        {}

        calibDone  = hsphySFR->CTRL.PHYREG[(uint32)phyIndex].DATA.U;
...
{
    pcieSFR->DSP.LTR.CTRL.B.RLTRMSK = 1u;
}


void IfxPcie_disableLtrInterrupt(IfxPcie *pcieSFR)
{
    pcieSFR->DSP.LTR.CTRL.B.RLTRMSK = 0u;
}
...
void IfxPcie_enableObffEventInterrupt(IfxPcie *pcieSFR, IfxPcie_ObffType obffType, IfxPcie_ObffEvent obffEvent)
{
    ...
    if (obffEvent != IfxPcie_ObffEvent_none)
    {
        ...
        {
            writeMask = 1U << (IFX_PCIE_USP_OBFFMSG_IDLEMSK_OFF + (uint32)(obffEvent * 3u));
            Ifx__ldmst(&(pcieSFR->USP.OBFFMSG), writeMask, writeMask);
        }
        else if (obffType == IfxPcie_ObffType_Wake)
        {
            writeMask = 1U << (IFX_PCIE_USP_OBFFWAKE_IDLEMSK_OFF + (uint32)(obffEvent * 3u));
...
void IfxPcie_enableObffInterrupts(IfxPcie *pcieSFR, IfxPcie_ObffType obffType)
{
    ...
    if (obffType == IfxPcie_ObffType_Msg)
    {
        writeMask = ((uint32)(1U << IFX_PCIE_USP_OBFFMSG_IDLEMSK_OFF)) | ((uint32)(1U << IFX_PCIE_USP_OBFFMSG_OBFFMSK_OFF)) | ((uint32)(1U << IFX_PCIE_USP_OBFFMSG_ACTMSK_OFF));
        Ifx__ldmst(&(pcieSFR->USP.OBFFMSG), writeMask, writeMask);
    }
    else if (obffType == IfxPcie_ObffType_Wake)
    {
        writeMask = ((uint32)(1U << IFX_PCIE_USP_OBFFWAKE_IDLEMSK_OFF)) | ((uint32)(1U << IFX_PCIE_USP_OBFFWAKE_OBFFMSK_OFF)) | ((uint32)(1U << IFX_PCIE_USP_OBFFWAKE_ACTMSK_OFF)) | ((uint32)(1U << IFX_PCIE_USP_OBFFWAKE_ERRMSK_OFF));
        Ifx__ldmst(&(pcieSFR->USP.OBFFWAKE), writeMask, writeMask);
    }
}
...
{
    IfxApApu_init((Ifx_ACCEN_ACCEN *)&MODULE_PCIE_DSP.ACCEN, apConfig);
}
