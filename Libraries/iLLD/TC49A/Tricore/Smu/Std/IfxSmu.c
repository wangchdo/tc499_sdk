...
void IfxSmu_initSafetyApConfig(IfxSmu_safetyApConfig *config)
{
...
void IfxSmu_initSafetyAp(Ifx_SMU_SAFE *smu, IfxSmu_safetyApConfig *config)
{
    /* Initialize the PROTs */
    IfxApProt_init((Ifx_PROT_PROT *)&smu->PROTSSE, &config->protseConfig);

    /* Change the state to CONFIG, Configure APU and set PROT state back to RUN */
    /* Initialize the APU */
    IfxApProt_setState((Ifx_PROT_PROT *)&smu->PROTSSE, IfxApProt_State_config);

    IfxApApu_init((Ifx_ACCEN_ACCEN *)&smu->ACCEN, &config->apuConfig);

    IfxApProt_setState((Ifx_PROT_PROT *)&smu->PROTSSE, IfxApProt_State_run);
}


void IfxSmu_initSharedAp(Ifx_SMU *smu, IfxSmu_sharedApConfig *config)
{
    /* Initialize the PROTs */
    IfxApProt_init((Ifx_PROT_PROT *)&smu->GCC.PROTGSE, &config->protseConfig);

    /* Change the state to CONFIG, Configure APU and set PROT state back to RUN */
    /* Initialize the APU */
    IfxApProt_setState((Ifx_PROT_PROT *)&smu->GCC.PROTGSE, IfxApProt_State_config);

    IfxApApu_init((Ifx_ACCEN_ACCEN *)&smu->GCC.ACCEN, &config->apuConfig);

    IfxApProt_setState((Ifx_PROT_PROT *)&smu->GCC.PROTGSE, IfxApProt_State_run);
}


void IfxSmu_initSharedApConfig(IfxSmu_sharedApConfig *config)
{
    IfxApProt_initConfig(&config->protseConfig);
...
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&MODULE_SMU.SAFE[0].PROTSSE, IfxApProt_State_config);
#endif
...
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&MODULE_SMU.SAFE[0].PROTSSE, IfxApProt_State_run);
#endif
}
...
void IfxSmu_initSecureApConfig(IfxSmu_secureApConfig *config)
{
    IfxApProt_initConfig(&config->protseConfig);
...
void IfxSmu_initSecureAp(Ifx_SMU *smu, IfxSmu_secureApConfig *config)
{
    /* Initialize the PROTs */
    IfxApProt_init((Ifx_PROT_PROT *)&smu->CS.PROTCSE, &config->protseConfig);

    /* Change the state to CONFIG, Configure APU and set PROT state back to RUN */
    /* Initialize the APU */
    IfxApProt_setState((Ifx_PROT_PROT *)&smu->CS.PROTCSE, IfxApProt_State_config);

    IfxApApu_init((Ifx_ACCEN_ACCEN *)&smu->CS.ACCEN, &config->apuConfig);

    IfxApProt_setState((Ifx_PROT_PROT *)&smu->CS.PROTCSE, IfxApProt_State_run);
}
