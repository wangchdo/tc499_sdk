...
void IfxPmsPm_initStandbyConfig(IfxPmsPm_StandbyConfig *standbyConfig)
{
    ...
    standbyConfig->scrCfg.scrClockSupply                              = IfxPmsPm_ScrClocking_100mhzOptional;
...
{
    config->powerDown.cpuA  = IfxPmsPm_PowerDown_disable;
    config->powerDown.cpuB  = IfxPmsPm_PowerDown_disable;
    config->powerDown.ppu   = IfxPmsPm_PowerDown_disable;
    config->powerDown.ememA = IfxPmsPm_PowerDown_disable;
    config->powerDown.ememB = IfxPmsPm_PowerDown_disable;
    config->powerDown.ememC = IfxPmsPm_PowerDown_disable;
    config->powerDown.ememD = IfxPmsPm_PowerDown_disable;
    config->cpuClkDivider   = IfxPmsPm_PowerSwitchCLockDivider_0;
    config->ppuClkDivider   = IfxPmsPm_PowerSwitchCLockDivider_0;
    config->ememClkDivider  = IfxPmsPm_PowerSwitchCLockDivider_0;
}
...
void IfxPmsPm_initandEnablePowerDown(Ifx_PMS *pms, IfxPmsPm_PowerGatingConfig *config)
{
    ...
    pdCon0.B.CLKCPU      = config->cpuClkDivider;
    pdCon0.B.CLKPPU      = config->ppuClkDivider;
    pdCon0.B.CLKEMEM     = config->ememClkDivider;
    /* Enable power down of the respective domain */
    pdCon0.B.CPUAPDEN_P  = 1;
    pdCon0.B.CPUAPDEN    = config->powerDown.cpuA;
    pdCon0.B.CPUBPDEN_P  = 1;
    pdCon0.B.CPUBPDEN    = config->powerDown.cpuB;
    pdCon0.B.PPUPDEN_P   = 1;
    pdCon0.B.PPUPDEN     = config->powerDown.ppu;
    pdCon0.B.EMEMAPDEN_P = 1;
    pdCon0.B.EMEMAPDEN   = config->powerDown.ememA;
    pdCon0.B.EMEMBPDEN_P = 1;
    pdCon0.B.EMEMBPDEN   = config->powerDown.ememB;
    pdCon0.B.EMEMCPDEN_P = 1;
    pdCon0.B.EMEMCPDEN   = config->powerDown.ememC;
    pdCon0.B.EMEMDPDEN_P = 1;
    pdCon0.B.EMEMDPDEN   = config->powerDown.ememD;
...
boolean IfxPmsPm_powerDownTrigger(Ifx_PMS *pms, IfxPmsPm_CorePowerDomain powerDomain)
{
    ...
    switch (powerDomain)
    {
    case IfxPmsPm_CorePowerDomain_cpuA:

        if (pms->PD.CON0.B.CPUAPDEN)
        {
            /* disabling alarm and reset trigger to avoid cold PORST when power down is triggered*/
            pms->VMONP.VDDCPUARST.U         |= (1u << IFX_PMS_VMONPRST_RESETOFF_P_OFF) | (1u << IFX_PMS_VMONPRST_RESETOFF_OFF);
            pms->VMONP.VDDCPUACON.B.UVENABLE = 0;
            pms->VMONS.VDDCPUACON.B.UVMOD    = 0;

            pdCon1.B.CPUAOFFREQ_P            = 1;
            pdCon1.B.CPUAOFFREQ              = 1;
            powerDownSuccesss                = 0;
        }

        break;
    case IfxPmsPm_CorePowerDomain_cpuB:

        if (pms->PD.CON0.B.CPUBPDEN)
        {
            /* disabling alarm and reset trigger to avoid cold PORST when power down is triggered*/
            pms->VMONP.VDDCPUBRST.U         |= (1u << IFX_PMS_VMONPRST_RESETOFF_P_OFF) | (1u << IFX_PMS_VMONPRST_RESETOFF_OFF);
            pms->VMONP.VDDCPUBCON.B.UVENABLE = 0;
            pms->VMONS.VDDCPUBCON.B.UVMOD    = 0;

            pdCon1.B.CPUBOFFREQ_P            = 1;
            pdCon1.B.CPUBOFFREQ              = 1;
            powerDownSuccesss                = 0;
        }
    ...
    case IfxPmsPm_CorePowerDomain_ppu:
        ...
        break;
    case IfxPmsPm_CorePowerDomain_ememA:

        if (pms->PD.CON0.B.EMEMAPDEN)
        {
            /* disabling alarm and reset trigger to avoid cold PORST when power down is triggered*/
            pms->VMONP.VDDLMURST.U         |= (1u << IFX_PMS_VMONPRST_RESETOFF_P_OFF) | (1u << IFX_PMS_VMONPRST_RESETOFF_OFF);
            pms->VMONP.VDDLMUCON.B.UVENABLE = 0;
            pms->VMONS.VDDLMUCON.B.UVMOD    = 0;

            pdCon1.B.EMEMAOFFREQ_P          = 1;
            pdCon1.B.EMEMAOFFREQ            = 1;
            powerDownSuccesss               = 0;
        }

        break;
    case IfxPmsPm_CorePowerDomain_ememB:

        if (pms->PD.CON0.B.EMEMBPDEN)
        {
            /* disabling alarm and reset trigger to avoid cold PORST when power down is triggered*/
            pms->VMONP.VDDLMURST.U         |= (1u << IFX_PMS_VMONPRST_RESETOFF_P_OFF) | (1u << IFX_PMS_VMONPRST_RESETOFF_OFF);
            pms->VMONP.VDDLMUCON.B.UVENABLE = 0;
            ...
            pdCon1.B.EMEMBOFFREQ_P          = 1;
            pdCon1.B.EMEMBOFFREQ            = 1;
            powerDownSuccesss               = 0;
        }

        break;
    case IfxPmsPm_CorePowerDomain_ememC:

        if (pms->PD.CON0.B.EMEMCPDEN)
        {
            /* disabling alarm and reset trigger to avoid cold PORST when power down is triggered*/
            pms->VMONP.VDDLMURST.U         |= (1u << IFX_PMS_VMONPRST_RESETOFF_P_OFF) | (1u << IFX_PMS_VMONPRST_RESETOFF_OFF);
            pms->VMONP.VDDLMUCON.B.UVENABLE = 0;
            ...
            pdCon1.B.EMEMCOFFREQ_P          = 1;
            pdCon1.B.EMEMCOFFREQ            = 1;
            powerDownSuccesss               = 0;
        }

        break;
    case IfxPmsPm_CorePowerDomain_ememD:

        if (pms->PD.CON0.B.EMEMDPDEN)
        {
            /* disabling alarm and reset trigger to avoid cold PORST when power down is triggered*/
            pms->VMONP.VDDLMURST.U         |= (1u << IFX_PMS_VMONPRST_RESETOFF_P_OFF) | (1u << IFX_PMS_VMONPRST_RESETOFF_OFF);
            pms->VMONP.VDDLMUCON.B.UVENABLE = 0;
            ...
            pdCon1.B.EMEMDOFFREQ_P          = 1;
            pdCon1.B.EMEMDOFFREQ            = 1;
            powerDownSuccesss               = 0;
        }
...
void IfxPmsPm_initandTriggerPowerDown(Ifx_PMS *pms, IfxPmsPm_PowerGatingConfig *config)
{
    ...
    if (config->powerDown.cpuA)
    {
        ...
        pms->VMONP.VDDCPUACON.B.UVENABLE = 0;
        pms->VMONS.VDDCPUACON.B.UVMOD    = 0;
        /* Request power domain switch off */
        pdCon1.B.CPUAOFFREQ_P            = 1;
        pdCon1.B.CPUAOFFREQ              = 1;
    }

    /* CPU B */
    if (config->powerDown.cpuB)
    {
        /* Enable Power Down */
        pdCon0.B.CPUBPDEN_P = 1;
        pdCon0.B.CPUBPDEN   = config->powerDown.cpuB;
    ...
    if (config->powerDown.ememA || config->powerDown.ememB || config->powerDown.ememC || config->powerDown.ememD)
    {
        /* disabling alarm and reset trigger to avoid cold PORST when power down is triggered*/
        pms->VMONP.VDDLMURST.U         |= (1u << IFX_PMS_VMONPRST_RESETOFF_P_OFF) | (1u << IFX_PMS_VMONPRST_RESETOFF_OFF);
        pms->VMONP.VDDLMUCON.B.UVENABLE = 0;
