...
void IfxCan_Can_initCre(IfxCan_Can_Node *node, IfxCan_CreConfig *config)
{
    ...
    /*Transmit Buffer*/
    cre->HBUF.TX.STAT.B.COUNT = 1; /*Writing nonzero value resets the counter*/
    cre->HBUF.TX.STAT.B.THREQ = 1; /*Clears the bit*/

    Ifx_CAN_N_CRE_HBUF_TX_CONFIG txBuf;
    txBuf.U     = 0;
    txBuf.B.LWM = config->txBufLastWriteMode;
    txBuf.B.LEN = config->txBufFixedDataLength;
    //txBuf.B.TRIGEN        = config->txBufDreTriggerEnable;

    cre->HBUF.TX.CONFIG.U = txBuf.U;
    ...
    if (config->interrupt.txBufferInterruptEnable)
    {
        /*Interrupt Node Init*/
        if ((config->interrupt.txHBuf.priority > 0) || (config->interrupt.txHBuf.typeOfService == IfxSrc_Tos_dma0) || (config->interrupt.txHBuf.typeOfService == IfxSrc_Tos_dma1))
        {
            IfxCan_Node_setGroupInterruptLine(node->node, IfxCan_InterruptGroup_txHBuf, config->interrupt.txHBuf.interruptLine);
            srcPointer = IfxCan_getSrcPointer(node->can, config->interrupt.txHBuf.interruptLine);
            IfxSrc_init(srcPointer, config->interrupt.txHBuf.typeOfService, config->interrupt.txHBuf.priority, config->interrupt.txHBuf.vmId);
            IfxSrc_enable(srcPointer);
        }

        cre->HBUF.TX.CONFIG.B.INTEN = TRUE;
    }
    else
    {
        /*DRE Trigger is enabled*/
        cre->HBUF.TX.CONFIG.B.INTEN = FALSE;
    }
...
void IfxCan_Can_Cre_readNewMessageBlock(IfxCan_Can_Node *node, IfxCan_CreRxHostBufferIndex index, IfxCan_CreRxBuffer *buffer)
{
    ...
    if (index == IfxCan_CreRxHostBufferIndex_0)
    {
        /*CRE Fifo Index 0*/
        ...
        while (cre->HBUF.RX[0].STAT.B.RHREQ == 0)
        {}

        buffer->isMessageAvailable = TRUE;

        if (cre->HBUF.RX[0].STAT.B.VRH)
        {
        ...
        if (cre->HBUF.RX[0].CONFIG.B.LRM == 0)
        {
            /*Dynamic Data Length*/
            uint8 length = rxBufPtr->R1.B.DLC;

            if (length > 0)
            {
                if (rxBufPtr->R1.B.FDF == 0)
                {
        ...
        {
            /*Fixed Data Length*/
            uint8 length = cre->HBUF.RX[0].CONFIG.B.LEN; /*Length of data in words*/
    ...
    else
    {
        /*CRE Fifo Index 1*/
        /*Wait until Valid Message is Available in buffer*/
        //while(cre->HBUF.RX[1].STAT.B.STATE != 2)
        //{}

        while (cre->HBUF.RX[1].STAT.B.RHREQ == 0)
        {}

        buffer->isMessageAvailable = TRUE;

        if (cre->HBUF.RX[1].STAT.B.VRH)
        {
            buffer->isRoutingValid      = TRUE;
            buffer->rxHostBuffer.UCRH.U = rxBufPtr->UCRH.U;
        }
        ...
        if (cre->HBUF.RX[1].CONFIG.B.LRM == 0)
        {
            /*Dynamic Data Length*/
            uint8 length = rxBufPtr->R1.B.DLC;
        ...
        {
            /*Fixed Data Length*/
            uint8 length = cre->HBUF.RX[1].CONFIG.B.LEN; /*Length of data in words*/
...
void IfxCan_Can_Cre_writeNewMessage(IfxCan_Can_Node *node, IfxCan_CreTxMsgConfig *config, uint32 *data)
{
    ...
    else
    {
        ...
        length = cre->HBUF.TX.CONFIG.B.LEN;
    ...
    //{}
    while (cre->HBUF.TX.STAT.B.THREQ == 1) /*Wait until Buffer is free and h/w sets it to 0 when free*/
    {}
...
IfxCan_Status IfxCan_Can_sendMessage(IfxCan_Can_Node *node, IfxCan_Message *message, uint32 *data)
{
    ...
    else
    {
        ...
        if (message->timeStampEnabled == TRUE)
        {
            IfxCan_Node_setTxEventFifoCtrl(txBufferElement, TRUE);
