...
void IfxQspi_SpiSlave_initModule(IfxQspi_SpiSlave *handle, const IfxQspi_SpiSlave_Config *config)
{
    ...
    if (config->dma.useDma)
    {
        ...
        /* Dma channel interrupt configuration */
        {
            ...
            IfxSrc_init(src, config->base.isrProvider, config->base.txPriority, config->base.vmId); /*Must be configurable*/
            IfxSrc_enable(src);

            src = IfxDma_getSrcPointer(dmaSFR, (IfxDma_ChannelId)config->dma.rxDmaChannelId);
            IfxSrc_init(src, config->base.isrProvider, config->base.rxPriority, config->base.vmId); /*Must be configurable*/
            IfxSrc_enable(src);
        }
    }
