...
void IfxQspi_SpiMaster_initModule(IfxQspi_SpiMaster *handle, const IfxQspi_SpiMaster_Config *config)
{
    ...
    if (config->dma.useDma)
    {
...

        {
            ...
            dmaCfg.channelVmId                   = config->base.vmId; /*Must be configurable*/
    ...
    /* Qspi interrupt configuration */
    {
        ...
        else
        {
            ...
            {
                volatile Ifx_SRC_SRCR *src = IfxQspi_getTransmitSrc(qspiSFR);
                IfxSrc_init(src, config->base.isrProvider, config->base.txPriority, config->base.vmId);
                IfxSrc_enable(src);
            }
            ...
            if (config->base.rxPriority != 0)
            {
                ...
                IfxSrc_init(src, config->base.isrProvider, config->base.rxPriority, config->base.vmId);
                IfxSrc_enable(src);
            }
            ...
            if (config->base.erPriority != 0)
            {
                ...
                IfxSrc_init(src, config->base.isrProvider, config->base.erPriority, config->base.vmId);
                IfxSrc_enable(src);
            }
        }
    }
...
void IfxQspi_SpiMaster_initModuleConfig(IfxQspi_SpiMaster_Config *config, Ifx_QSPI *qspi)
{
    ...
    config->dma.dmaIndex                       = IfxDma_Index_0;
    config->base.maximumBaudrate               = 50000000;
    config->triggerConfig.hardwareTiggerEnable = IfxQspi_HardwareTrigger_disable;       /*Hardware Trigger source disabled*/
    config->triggerConfig.triggerSource        = IfxQspi_HardwareTriggerSource_gtm;
}
