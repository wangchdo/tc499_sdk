...
uint32 IfxGtm_Tom_Tgc_buildFeature(uint16 enableMask, uint16 disableMask, uint8 bitfieldOffset)
{
    ...
    for (i = 0u; i < IFXGTM_TOM_NUM_TGC_CHANNELS; i++)
    {
        ...
        if ((mask & 0x1u) == 0x1u)
        {
            reg |= (uint32)((uint32)IfxGtm_FeatureControl_enable << shift);
        }

        if ((mask & 0x10000u) == 0x10000u)
        {
            reg |= (uint32)((uint32)IfxGtm_FeatureControl_disable << shift);
        }
...
float32 IfxGtm_Tom_Ch_getFrequency(Ifx_GTM_CLS_TOM *tom, IfxGtm_Tom_Ch channel)
{
    float32                  frequency         = 0.0f, sourceFrequency;
    uint32                   periodTicks       = 0u;
    IfxGtm_Tom_Ch_ResetEvent counterResetEvent = (IfxGtm_Tom_Ch_ResetEvent)tom->CH[channel].CTRL.B.RST_CCU0;

    while ((counterResetEvent != IfxGtm_Tom_Ch_ResetEvent_onCm0) &&
           (channel > IfxGtm_Tom_Ch_0))       /* Counter is reset from previous channel */
    {
        channel--;
        counterResetEvent = (IfxGtm_Tom_Ch_ResetEvent)tom->CH[channel].CTRL.B.RST_CCU0;
    }

    if (counterResetEvent == IfxGtm_Tom_Ch_ResetEvent_onCm0)  /* Counter is reset from own channel */
    {
        /* 1. Get channel input frequency */
        sourceFrequency = IfxGtm_Tom_Ch_getClockFrequency(&MODULE_GTM, tom, channel);
...
float32 IfxGtm_Tom_Ch_getDuty(Ifx_GTM_CLS_TOM *tom, IfxGtm_Tom_Ch channel)
{
    float32                  duty = 0.0f;
    uint32                   cm0Reg, cm1Reg, periodReg;
    IfxGtm_Tom_Ch_ResetEvent counterResetEvent;
    ...
    if (counterResetEvent == IfxGtm_Tom_Ch_ResetEvent_onCm0)  /* Period is determined by own channel */
    {
        periodReg = cm0Reg;
    }
    else    /* Period is determined by some previous channel */
    {
        ...
        while ((tempChannel > 0) &&
               (tom->CH[tempChannel].CTRL.B.RST_CCU0 != (uint8)IfxGtm_Tom_Ch_ResetEvent_onCm0))
        {
            tempChannel--;
        }
    ...
    if (tom->CH[channel].CTRL.B.UDMODE == 0u)                    /* Channel counter running in continuous counting up mode */
    {
        if (counterResetEvent == IfxGtm_Tom_Ch_ResetEvent_onCm0) /* Duty exists during CN0 = [0, CM1] */
        {
