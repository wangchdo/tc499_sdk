 ...
 *
 *
 * \defgroup IfxLld_Gtm_Tim_In_Usage How to use the TIM Input Interface layer?
 * \ingroup IfxLld_Gtm_Tim_In
 *
 * \section IfxLld_Gtm_Tim_In_Overview Overview
 *
 * The TIM Input interface layer is used for usecases such as PWM measurement. It uses the TIM submodule of GTM to perform this measurement.
 *
 * \section IfxLld_Gtm_Tim_In_Code Coding Example
 *
 ...
 *
 * \subsection IfxLld_Gtm_Tim_In_Step1 Step 1: Enable GTM and its clocks
 * Please enable the GTM module and enable the required clocks for the TIM submodule.
 *
 * \subsection IfxLld_Gtm_Tim_In_Step2 Step 2: Instantiate variables
 *
 *     \code
 *     // Include interface
 ...
 *     IfxGtm_Tim_In          timIn;          // Driver handle
 *     IfxGtm_Tim_In_Config   timInConfig;    // Configuration structure
 *     \endcode
 *
 * \subsection IfxLld_Gtm_Tim_In_Step3 Step 3: [OPTIONAL] Define interrupt handler
 *
 *     \code
 ...
 *         IfxGtm_Tim_In_onIsr(&timIn);
 *         timIsrCounter += 1;
 *     }
 *     \endcode
 *
 * \subsection IfxLld_Gtm_Tim_In_Step4 Step 4: Initalize TIM
 *
 *     \code
 ...
 *     timInConfig.cluster                      = IfxGtm_Cluster_0;
 *     timInConfig.channelIndex                 = IfxGtm_Tim_Ch_0;
 *     // Configure Interrupt parameters
 *     timInConfig.irqMode                      = IfxGtm_IrqMode_pulseNotify;
 ...
 *     // Configure the Input
 *     timInConfig.filter.input                 = IfxGtm_Tim_In_Input_currentChannel;
 *     timInConfig.filter.inputPin              = &IfxGtm_TIM0_0_P02_0_IN;
 *     timInConfig.filter.inputPinMode          = IfxPort_InputMode_pullDown;
 *
 *     // Initialize the TIM Channel now
 *     IfxGtm_Tim_In_init(&timIn, &timInConfig);
 *
 *     \endcode
 *
 * Now, whenever a new measurement is captured, the function IfxGtm_Tim_In_onIsr is called. This will update the value of frequency and duty in handle
 *
 * \subsection IfxLld_Gtm_Tim_In_Step5 Step 5: Read measurement
 *
 *     \code
 ...
 *
 *         // Get measurements
 *         float32 measuredPwmFrequency    = 1.0f / IfxGtm_Tim_In_getPeriodSecond(&timIn);
 *         float32 measuredDutyPercent     = IfxGtm_Tim_In_getDutyPercent(&timIn, &dataCoherent);
 *     }
 *     \endcode
 *
...
typedef struct
{
    Ifx_GTM_CLS_TIM_CH *channel;                     /**< \brief TIM channel used */
    ...
    IfxGtm_Cluster      clsIndex;                    /**< \brief Index of the CLS module being used. */
...
typedef struct
{
    ...
    IfxGtm_Cluster              cluster;            /**< \brief CLS index */
...
IFX_INLINE float32 IfxGtm_Tim_In_getPeriodSecond(IfxGtm_Tim_In *driver)
{
    return (float32)IfxGtm_Tim_In_getPeriodTicks(driver) / driver->captureClockFrequency;
}
