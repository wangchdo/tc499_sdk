...
boolean IfxGtm_Input_selectGethTimInput(IfxGtm_GethTimInput input, IfxGtm_Tim cluster, IfxGtm_Tim_Ch channel)
{
    boolean result;
    uint32  inputEntry = IFXGTM_CFG_GETINPUTENTRY(cluster, channel, input, 0u);

    result = IfxGtm_Input_selectPeripheralTimInput(inputEntry, &IfxGtm_Cfg_GethTimInputSel[0]);
