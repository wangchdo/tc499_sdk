...
boolean IfxGtm_Atom_Pwm_init(IfxGtm_Atom_Pwm_Driver *driver, const IfxGtm_Atom_Pwm_Config *config)
{
    ...
    if (config->mode == IfxGtm_Atom_Mode_outputPwm)
    {
        result = TRUE;

        Ifx_GTM_CLS_ATOM    *atomSFR = &config->gtm->CLS[config->cluster].ATOM;
        Ifx_GTM_CLS_ATOM_CH *channel = &atomSFR->CH[config->atomChannel];
        ...
        IfxGtm_Atom_Agc_enableChannel(driver->agc, config->atomChannel, TRUE, FALSE);

        /* Set channel to start PWM output when trigger is received */
        IfxGtm_Atom_Agc_enableChannelOutput(driver->agc, config->atomChannel, TRUE, FALSE);

        /* Set Signal Polarity value here */
        IfxGtm_Atom_Ch_setSignalLevel(atomSFR, driver->atomChannel, config->signalLevel);

        /* Set ATOM signal output mode as PWM */
        IfxGtm_Atom_Ch_setMode(atomSFR, driver->atomChannel, config->mode);

        /* Set ATOM channel clock source */
        IfxGtm_Atom_Ch_setClockSource(atomSFR, driver->atomChannel, config->clock);

        /* Reset counter to 0 */
        IfxGtm_Atom_Ch_setCounterValue(atomSFR, driver->atomChannel, 0);
        ...
        if ((config->interrupt.ccu0Enabled == TRUE) || (config->interrupt.ccu1Enabled == TRUE))
        {
            ...
            src = IfxGtm_Atom_Ch_getSrcPointer(config->gtm, config->cluster, config->atomChannel);
        ...
        if (config->synchronousUpdateEnabled == TRUE)
        {
            IfxGtm_Atom_Ch_setCompareZeroShadow(atomSFR, driver->atomChannel, period);
            IfxGtm_Atom_Ch_setCompareOneShadow(atomSFR, driver->atomChannel, dutyCycle);
        }
        else
        {
            IfxGtm_Atom_Ch_setCompareZero(atomSFR, driver->atomChannel, period);
            IfxGtm_Atom_Ch_setCompareOne(atomSFR, driver->atomChannel, dutyCycle);
        }
        ...
        {
            (void)IfxGtm_ConnectToMsc(config->cluster, IfxGtm_TrigSource_atom, (IfxGtm_TrigChannel)config->atomChannel, config->mscOut);
        }
...
{
    const IfxGtm_Atom_Pwm_Config defaultConfig = {
        .gtm         = NULL_PTR,
        .cluster     = IfxGtm_Cluster_0,
        .atomChannel = IfxGtm_Atom_Ch_0,
