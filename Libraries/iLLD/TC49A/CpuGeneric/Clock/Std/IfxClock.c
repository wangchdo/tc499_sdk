...
#if !defined(IFX_CLOCK_DISABLE_RAMPCON)
    {IfxClock_RampWaitTime_1us,                                IfxClock_RampFreqStepSize_20MHz,400},
#endif
#if defined(IFX_CLOCK_DISABLE_RAMPCON)
    {
...
IFX_CONST IfxClock_Mod_Config IfxClock_defaultModConfig = {
    ...
    IfxClock_ModulationAmplitude_0p5, /* default 0 value */
};
...
boolean IfxClock_configureCcuPll(const IfxClock_Config *pllCfg)
{
    ...
    if ((CLOCK_OSCSTAT.B.SOWDS != 0U) || (CLOCK_OSCSTAT.B.POWDS != 0U))
    {
        initError = 1U;
    }
        ...
        (pllCfg->PerClockSourceSelect != IfxClock_PerClockSourceSelect_back))
    {
        ...
        if (pllCfg->SysClockSourceSelect == IfxClock_SysClockSourceSelect_pll)
        {
            ...
            timeoutCycleCount  = IFXCLOCK_SYSPLLSTAT_PWDSTAT_TIMEOUT_COUNT;
            ...
            /* Step3: Reset Lock detection for System PLL */
            {
                timeoutCycleCount = IFXCLOCK_CCUCON_LCK_BIT_TIMEOUT_COUNT;
                ...
                CLOCK_SYSPLLCON0.B.RESLD = 1; /* Restart Sys PLL lock detection */
            }
        }
        ...
        if (pllCfg->PerClockSourceSelect == IfxClock_PerClockSourceSelect_pll)
        {
            ...
            /* Step3: Reset Lock detection for PER PLL */
            {
                timeoutCycleCount = IFXCLOCK_CCUCON_LCK_BIT_TIMEOUT_COUNT;
...

    {
        ...
        rampcon.B.SZ      = pllCfg->rampConfig.rampFreqStepSize;       //the step size in 10 MHz steps
        rampcon.B.WAIT    = pllCfg->rampConfig.rampWaitTime;
...
uint32 IfxClock_getPllFrequency(void)
{
    ...
    oscFreq = IfxClock_getOscFrequency();
    freq    = (oscFreq * (clock->SYSPLLCON0.B.NDIV + 1)) / ((clock->SYSPLLCON0.B.PDIV + 1) * ((clock->SYSPLLCON1.B.K2DIV + 1))) + 0.5f;
    return (uint32)freq;
}
...
boolean IfxClock_switchToBackupClock(const IfxClock_Config *cfg)
{
     ...
     * Enable clock source (CCUCON.CLKSELS = RAMP) to RAMPCON frequency. */
    {
        ...
        rampcon.B.SZ   = cfg->rampConfig.rampFreqStepSize; //the step size in 10 MHz steps
        rampcon.B.WAIT = cfg->rampConfig.rampWaitTime;
...
void IfxClock_initAp(Ifx_CLOCK *clock, IfxClock_ApConfig *config)
{
    /* Initialize the PROTs */
    IfxApProt_init((Ifx_PROT_PROT *)&clock->PROTSE, &config->protseConfig);

    /* Change the state to CONFIG, Configure APU and set PROT state back to RUN */
    /* Initialize the APU */
    IfxApProt_setState((Ifx_PROT_PROT *)&clock->PROTSE, IfxApProt_State_config);
    IfxApApu_init((Ifx_ACCEN_ACCEN *)&clock->ACCEN, &config->apuConfig);
    IfxApProt_setState((Ifx_PROT_PROT *)&clock->PROTSE, IfxApProt_State_run);
}


void IfxClock_initApConfig(IfxClock_ApConfig *config)
{
