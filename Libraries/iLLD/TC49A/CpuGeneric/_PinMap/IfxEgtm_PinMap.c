...
void IfxEgtm_PinMap_setHrpwmOut(IfxEgtm_Hrpwm_Out *config, IfxPort_OutputMode outputMode, IfxPort_PadDriver padDriver)
{
    IfxPort_setPinModeOutput(config->pin.port, config->pin.pinIndex, outputMode, config->select);
    IfxPort_setPinPadDriver(config->pin.port, config->pin.pinIndex, padDriver);
#ifdef IFXEGTM_CFG_ENABLE_PCSRSEL_WORKAROUND
    IfxPort_Index portIndex = IfxPort_getIndex(config->pin.port);
    uint8         pcsrSel   = config->pin.pinIndex;

    if (portIndex == IfxPort_Index_02)
    {
        /* Exp: PCSRSEL.B.PCSR2	Actual: PCSRSEL.B.PCSR1 */
        if (config->pin.pinIndex == 2)
        {
            pcsrSel = 1;
        }
        /* Exp: PCSRSEL.B.PCSR4	Actual: PCSRSEL.B.PCSR2 */
        else if ((config->pin.pinIndex >= 4) && (config->pin.pinIndex <= 9))
        {
            pcsrSel = config->pin.pinIndex - 2;
        }
    }
    else if (portIndex == IfxPort_Index_20)
    {
        if (config->pin.pinIndex == 7)
        {
            pcsrSel = 14;
        }
        else
        {
            pcsrSel = config->pin.pinIndex - 1;
        }
    }

    IfxPort_setPinControllerSelection(config->pin.port, pcsrSel);
#else
