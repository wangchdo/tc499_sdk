...
/** \brief Configures the Integrator of the Cdsp core
 * \param dsp Pointer to DSP core sfr
 * \param integratorCfg Pointer to the Integrator Configuration
 * \return None
 */
IFX_STATIC void IfxAdc_Cdsp_setupIntegrator(Ifx_ADC_CDSP_DSP *dsp, IfxAdc_Cdsp_IntegratorConfig *integratorCfg);
...
IFX_STATIC void IfxAdc_Cdsp_configureBoundary(Ifx_ADC_CDSP_DSP *dsp, IfxAdc_Cdsp_BoundaryConfig *config)
{
    ...
    if (config != NULL_PTR)
    {
        ...
        bndCfg.B.SR2GT     = config->srvReqGateEnable;
...
IFX_STATIC void IfxAdc_Cdsp_setupTriggers(Ifx_ADC_CDSP_DSP *dsp, IfxAdc_Cdsp_TriggerConfig *triggerCfg)
{
    ...
    if (triggerCfg != NULL_PTR)
    {
        ...
        /* Integrator trigger configuration */
        if (triggerCfg->integratorTrig != NULL_PTR)
        {
            Ifx_ADC_CDSP_DSP_INTTRCFG intTrCfg;
            intTrCfg.B.INTRDLY  = triggerCfg->integratorTrig->delay;
            intTrCfg.B.INTRM    = triggerCfg->integratorTrig->edgeSel;
            intTrCfg.B.INTRS    = triggerCfg->integratorTrig->triggerSel;
            coreSfr->INTTRCFG.U = intTrCfg.U;
        }
    }
}
...
{
    Ifx_ADC_CDSP_DSP       *coreSfr = dsp;
    Ifx_ADC_CDSP_DSP_INTCFG intCfg;

    intCfg.U       = coreSfr->INTCFG.U;

    intCfg.B.INTEN = integratorCfg->enableIntegrator;

    if (integratorCfg->windowSizeControl == IfxAdc_CdspIntegratorWindowControl_external)
    {
        intCfg.B.IWS = 1u; /* Externally Controlled end of integration*/
    }
    else
    {
        intCfg.B.IWS = 0u; /* Internally Controlled end of integration*/
    }

    coreSfr->INTCFG.U = intCfg.U;
}
