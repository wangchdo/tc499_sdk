...
void IfxAdc_Tmadc_initModule(IfxAdc_Tmadc *tmadc, const IfxAdc_Tmadc_Config *config)
{
    ...
    {
        tmadc->emuxChannel = emuxcfgPtr->emuxChannel;
    }
...
void IfxAdc_Tmadc_initChannel(IfxAdc_Tmadc_Ch *channel, IfxAdc_Tmadc_ChConfig *config)
{
    ...
    channel->rsltAXI          = &(MODULE_ADC_AXI.TMADCRES[modId].RESAXI[rsltReg]);
    channel->tmstmpAXI        = &(MODULE_ADC_AXI.TMADCRES[modId].TSAXI[rsltReg]);
...
uint16 IfxAdc_Tmadc_readChannelResult(IfxAdc_Tmadc_Ch *channel)
{
    ...
    return channel->rsltAXI->B.RESULT;     /* read result LLI*/
#else
...
void IfxAdc_Tmadc_configureEmux(Ifx_ADC_TMADC *tmadc, IfxAdc_Tmadc_EmuxConfig *emuxConfig)
{
    ...
    IfxAdc_Tmadc_TriggerConfig *emuxtrigPtr = emuxConfig->trigger;
    uint32                      delay       = 0u;
    /* Reading register */
    Ifx_ADC_TMADC_EMUXCFG       emuxCfg;
    Ifx_ADC_TMADC_EMUXTCFG      emuxtriggerCfg;
    emuxtriggerCfg.U = sfrPtr->EMUXTCFG.U;
    emuxCfg.U        = sfrPtr->EMUXCFG.U;

    /* EMUX Configuration */
    emuxCfg.B.MODE   = emuxConfig->mode;
    emuxCfg.B.CHUPD  = emuxConfig->chnlUpdate;
    emuxCfg.B.CHNR   = emuxConfig->emuxChannel;
    ...
    /* Emux trigger configuration */
    if (emuxtrigPtr != NULL_PTR)
    {
        delay                 = IfxAdc_getTmadcTriggerDelayValue(emuxtrigPtr->delayNS);
        emuxtriggerCfg.B.TRS  = emuxtrigPtr->hwTrigger.muxSel;
        emuxtriggerCfg.B.TRMS = emuxtrigPtr->hwTrigger.edgeSel;
        emuxtriggerCfg.B.TRD  = delay;
    }

    sfrPtr->EMUXTCFG.U = emuxtriggerCfg.U;
...
uint16 IfxAdc_Tmadc_readTimestamp(IfxAdc_Tmadc_Ch *channel)
{
    uint16 timeStamp = 0;
#if defined(IFX_ILLD_PPU_USAGE)
    timeStamp = channel->tmstmpAXI->B.TIMESTAMP; /* read timestamp LLI */
#else
...
#if defined(IFX_ILLD_PPU_USAGE)
    result.result    = channel->rsltAXI->B.RESULT;          /* read result LLI*/
    result.timeStamp = channel->tmstmpAXI->B.TIMESTAMP;     /* read timestamp LLI */
#else
...
void IfxAdc_Tmadc_initMonitorChannel(IfxAdc_Tmadc_MonitorCh *channel, IfxAdc_Tmadc_MonitorChannelConfig *config)
{
    ...
    channel->rsltAXI  = &(MODULE_ADC_AXI.TMADCRES[modId].MRESAXI[chId]);
    channel->moduleId = (IfxAdc_TmadcModule)modId;
