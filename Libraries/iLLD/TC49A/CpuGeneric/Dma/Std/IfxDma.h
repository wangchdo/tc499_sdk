...
typedef enum
{
    ...
    IfxDma_ChannelMoveSize_512bit = 6   /**< \brief 1 DMA move is 512 bit wide */
...
typedef enum
{
    ...
    IfxDma_HardwareResourcePartition_4,          /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_5,          /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_6,          /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_7,          /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_8,          /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_9,          /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_10,         /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_11,         /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_12,         /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_13,         /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_14,         /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
    IfxDma_HardwareResourcePartition_15          /**< \brief "Set of DMA channels are associated with hardware resource partition " + str(x)  */
 ...
 * \param dma pointer to DMA module
 * \param hardwareResourcePartition DMA hardware resource partition
 ...
 *                                                  IFXDMA_ERROR_TCSC |  // TCS Configuration error
 *                                                  IFXDMA_ERROR_DLL);  // failed linked list load error
 * \endcode
 *
 */
IFX_INLINE void IfxDma_clearErrorFlags(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition, uint32 mask);
 ...
 *
 */
IFX_INLINE void IfxDma_disableMoveEngineDestinationError(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition);
 ...
 *
 */
IFX_INLINE void IfxDma_disableMoveEngineSourceError(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition);
 ...
 *
 */
IFX_INLINE void IfxDma_enableMoveEngineDestinationError(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition);
 ...
 *
 */
IFX_INLINE void IfxDma_enableMoveEngineSourceError(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition);
 ...
 *           }
 *
 *      if( errorFlags & IFXDMA_ERROR_TCSC ) { // safe linked list CRC checksum error
 *           // ...
 *           }
 *
 ...
 *
 */
IFX_INLINE uint32 IfxDma_getErrorFlags(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition);
...
/** \brief Configure the hardware resource partition. Use the PROT APIs to modify the state to config before using this API
 * \param dma pointer to DMA module
 * \param channelId DMA channel number
...
/** \brief Enables the DMA Channel Resource Partition Valid bit
 * \param dma DMA module pointer
 * \param channelId Dma channel Id
 * \param resourcePartition DMA hardware resource partition
 * \return None
 */
IFX_INLINE void IfxDma_enableChannelHardwareResourcePartition(Ifx_DMA *dma, IfxDma_ChannelId channelId, IfxDma_HardwareResourcePartition resourcePartition);

/** \brief Disables the DMA Channel Resource Partition Valid bit
 * \param dma DMA module pointer
 * \param channelId Dma channel Id
 * \param resourcePartition DMA hardware resource partition
 * \return None
 */
IFX_INLINE void IfxDma_disableChannelHardwareResourcePartition(Ifx_DMA *dma, IfxDma_ChannelId channelId, IfxDma_HardwareResourcePartition resourcePartition);

/******************************************************************************/
/*-------------------------Global Function Prototypes-------------------------*/
/******************************************************************************/

/** \brief API to get the resource index of the DMA specified.
 * \param dma DMA module pointer
 * \return return dma index
 */
IFX_EXTERN IfxDma_Index IfxDma_getIndex(Ifx_DMA *dma);

/** \brief Configures access to all masters to all the DMAs in the device
 * \param apConfig config pointer to configuration structure
 * \return None
 */
IFX_EXTERN void IfxDma_configureAccessToDmas(IfxApApu_ApuConfig *apConfig);
...
IFX_INLINE volatile Ifx_SRC_SRCR *IfxDma_getSrcPointer(Ifx_DMA *dma, IfxDma_ChannelId channelId)
{
    ...
    if (dma == &MODULE_DMA0)
    {
        srcPtr = &MODULE_SRC.DMA0.CH[channelId];
    }

#if IFXDMA_NUM_MODULES > 1
    else
    {
        srcPtr = &MODULE_SRC.DMA1.CH[channelId];
    }
#endif
    return srcPtr;
}
...
IFX_INLINE void IfxDma_setSleepMode(Ifx_DMA *dma, IfxDma_SleepMode mode)
{
    ...
    IfxApProt_setState((Ifx_PROT_PROT *)&(dma->PROTSE), IfxApProt_State_config);
#endif
    dma->CLC.B.EDIS = mode;
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&(dma->PROTSE), IfxApProt_State_run);
#endif
}
...
{
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&(dma->RP[resourcePartition].PROT), IfxApProt_State_config);
#endif
    dma->HRR[channelId].B.HRP               = resourcePartition;
    dma->HRR[channelId].B.HRPV              = 1U;
    dma->RP[resourcePartition].MODE.B.VALID = 1U;
#if (IFX_PROT_ENABLED == 1U)
    IfxApProt_setState((Ifx_PROT_PROT *)&(dma->RP[resourcePartition].PROT), IfxApProt_State_run);
#endif
}
...
IFX_INLINE volatile Ifx_SRC_SRCR *IfxDma_getErrPointer(Ifx_DMA *dma, IfxDma_HardwareResourcePartition resourcePartition)
{
    ...
    if (dma == &MODULE_DMA0)
    {
        srcPtr = &MODULE_SRC.DMA0.ERR[resourcePartition];
    }

#if IFXDMA_NUM_MODULES > 1
    else
    {
        srcPtr = &MODULE_SRC.DMA1.ERR[resourcePartition];
    }
#endif
    return srcPtr;
}
...
IFX_INLINE void IfxDma_clearErrorFlags(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition, uint32 mask)
{
    dma->RP[hardwareResourcePartition].CLRE.U = mask;
}


IFX_INLINE void IfxDma_disableMoveEngineDestinationError(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition)
{
    dma->RP[hardwareResourcePartition].EER.B.EDER = 0U;
}


IFX_INLINE void IfxDma_disableMoveEngineSourceError(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition)
{
    dma->RP[hardwareResourcePartition].EER.B.ESER = 0U;
}


IFX_INLINE void IfxDma_enableMoveEngineDestinationError(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition)
{
    dma->RP[hardwareResourcePartition].EER.B.EDER = 1U;
}


IFX_INLINE void IfxDma_enableMoveEngineSourceError(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition)
{
    dma->RP[hardwareResourcePartition].EER.B.ESER = 1U;
}


IFX_INLINE uint32 IfxDma_getErrorFlags(Ifx_DMA *dma, IfxDma_HardwareResourcePartition hardwareResourcePartition)
{
    return dma->RP[hardwareResourcePartition].ERRSR.U;
}
